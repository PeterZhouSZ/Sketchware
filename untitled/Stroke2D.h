...
void Stroke2D<Point>::resample(int target_num_points) {
	...
	//		auto p = point(p0, p1, w0);
	//		result.push_back(p);
	//		edges.push_back(curve2dedge(std::make_pair<int, int>(index - 1, index), result[index-1].distance(result[index]));
	//		index++;
	...
	//edges.push_back(curve2dedge(std::make_pair<int, int>(index, index + 1), result[index - 1].distance(result[index]));
...
bool Stroke2D<Point>::tangent_test_2D() {
	...
	for (int i = 1; i < size() - 1; i++) {
		auto x = _points[i] - _points[i - 1];
		auto y = (_points[i + 1] - _points[i];
		auto z = _points[i + 1] - _points[i - 1];
		auto x2d = x.head<2>();
		auto y2d = y.head<2>();
		auto z2d = z.head<2>();
