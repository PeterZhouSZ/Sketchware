...
void Stroke3DShellProjection::set_heights_when_endpoints_are_known(const PointNormal & c1, const PointNormal & cn, const Stroke2D<Point2D> & stroke) {
...
boost::optional<Stroke3D::PointNormal> Stroke3DShellProjection::get_point_when_projectable(const Point2D & point, double height)const {
	auto position = unproject(Vector3d(point.x(), point.y(), 1)); 
	
	//Move forward the  camera
	auto d = core.camera->eye_to_center();
	d.normalize();
	d = d * height;
	auto eye_forward_position = (core.camera->get_eye() + d).eval();
	
	auto interest_position = core.intersect_convert(core.intersect_directly(merge(eye_forward_position, position - core.camera->get_eye())));
...
Polyline_PointNormal Stroke3DShellProjection::polyline_pn(const Stroke2D<Point2D> & stroke) {
	...
	set_heights_when_endpoints_are_known(*first_end_point, *last_end_point, stroke);
	...
	while (stroke_itr != stroke.end() - 1)
	{
		...
		//Store the successful projection points
		while (stroke_itr != stroke.end() - 1) {
			auto point = get_point_when_projectable(*stroke_itr, height(current_height_index));
			current_height_index += 1; stroke_itr++;
		...
		last_success_point = succeed_to_project.back();
		...
		fail_to_project.clear();
		succeed_to_project.clear();
	}
...
boost::optional<PointNormal> Stroke3DShellProjection::get_end_point_of_the_stroke(const Point2D & stroke_endpoint) {
	...
	auto result = Stroke3DShellContour::nearest_point_line_to_model(line);
	auto weight = DEFAULT_HEIGHT / result.tail<3>().norm();
	
	auto mat_v = merge(Vector3d::Identity().eval(), Vector3d(weight, weight, weight));
	auto mat = mat_v.asDiagonal();
	auto pn = (mat * result).eval();
	return pn;
}
