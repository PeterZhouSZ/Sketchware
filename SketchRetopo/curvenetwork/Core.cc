...
double curvenetwork::Core::find_snap_nearest_point(SnapType snapType, Vertex* v_min, Vertex*& v_snap) {
	...
	for (auto v = vertices.begin(); v != vertices.end(); ++v) {
		...
		if (snapType == SNAPTYPE_OPENEND)   { if (!is_openend)              continue; }
		else
			if (snapType == SNAPTYPE_CORNER)   { if (is_openend || !is_corner) continue; }
			else
				/* (snapType == SNAPTYPE_SIDE   )*/ {
				if (is_openend || is_corner) continue;
			}

		double dist = pn_norm(v_snap->pn - v->pn);
		if (dist < dist_min) {
			dist_min = dist;
			v_min = static_cast<Vertex*>(&*v);
		}
	}
...
double curvenetwork::Core::find_snap_nearest_point(SnapType snapType, Vertex* v_min, kt84::PointNormal & d_snap) { // d_snap  is  a segement from eye to surface
	double dist_min = DBL_MAX;

	auto nearest_point_line_to_point = [&](const PointNormal & line, const Vector3d & point) {
	...
	auto nearest_point_segment_line_to_point = [&](const PointNormal & segment_line, const Vector3d & point) {
		auto line_point1 = segment_line.head<3>().eval();
		auto line_point2 = segment_line.tail<3>().eval();
		PointNormal line;
		line << line_point1, line_point2 - line_point1;
		auto p = nearest_point_line_to_point(line, point).eval();
	...
	for (auto v = vertices.begin(); v != vertices.end(); ++v) {
		...
		if (snapType == SNAPTYPE_OPENEND)   { if (!is_openend)              continue; }
		else if (snapType == SNAPTYPE_CORNER)   { if (is_openend || !is_corner) continue; }
		else /* (snapType == SNAPTYPE_SIDE   )*/ { if (is_openend || is_corner) continue; }

		

		
		auto v_point = v->pn.head<3>().eval();
		auto temp = nearest_point_segment_line_to_point(d_snap, v_point);
		double dist = (temp - v_point).norm();
		if (dist < dist_min) {
			dist_min = dist;
			v_min = &*v;
		}
...
double curvenetwork::Core::find_snap_nearest_point(Vertex* v_min, kt84::PointNormal & d_snap) {
	double dist_min;
	SnapType snapType = SNAPTYPE_OPENEND;
	for (int i = 0; i < 3; i++) {
		dist_min = find_snap_nearest_point(snapType, v_min, d_snap);
		if (v_min != nullptr) return dist_min;
		snapType = (SnapType)(snapType + 1);
	}

	return dist_min;
}

double curvenetwork::Core::find_snap_nearest_point(Vertex* v_min, Vertex*& v_snap) {
	double dist_min;
	SnapType snapType = SNAPTYPE_OPENEND;
	for (int i = 0; i < 3; i++) {
		dist_min = find_snap_nearest_point(snapType, v_min, v_snap);
		if (v_min != nullptr) return dist_min;
		snapType = (SnapType)(snapType + 1);
	}
...
bool curvenetwork::Core::calc_snap(SnapType snapType, Vertex*& v_snap, Vertex* v_other, Halfedge*& h_snap, double dist_snap, bool corner_at_openend) {
	...
	double dist_min = find_snap_nearest_point(snapType, v_min, v_snap);
...
void curvenetwork::Core::render_edgechains_num_subdiv() const {
    ...
    glBegin(GL_POINTS);
    for (auto& e : edgechains) {
        if (e.is_deleted || e.halfchain[0]->patch || e.halfchain[1]->patch) continue;
