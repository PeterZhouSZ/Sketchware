...
namespace util {
    ...
    inline T top_and_pop(std::stack<T, Container>& s) {
        auto result = s.top();
        s.pop();
        return result;
    }
    template <typename T> inline std::pair<T, T> make_ordered_pair(const T& t0, const T& t1) {
        return t0 < t1 ? std::make_pair(t0, t1) : std::make_pair(t1, t0);
    }
    template <typename Target, typename Source> inline Target lexical_cast(const Source& src) {
        std::stringstream ss;
        ss << src;
        Target tgt;
        ss >> tgt;
    ...
    template <typename T> inline T max(const std::pair<T, T>& p) { return std::max<T>(p.first, p.second); }
    template <typename T> inline T min(const std::pair<T, T>& p) { return std::min<T>(p.first, p.second); }
    ...
    auto random_elem(const Container& c) -> decltype(*c.begin()) {
        return c[random_integral<decltype(c.size())>(0, c.size() - 1)];
    }
    ...
    template <typename T>
    inline T return_msg(const T& return_val, const char* msg) {
        std::cout << msg << std::endl;
        return return_val;
    }
    inline void return_msg(const char* msg) {
        std::cout << msg << std::endl;
    }
}
